...
def validate_barcode(data: str, barcode_type: BarCode) -> None:
    ...
    data_len = len(data)
    range_: List[int] = [_range0, _range1, _range2, _range3][range_type]()  # type: ignore
    ...
    if barcode_type is BarCode.ITF and data_len % 2 != 0:
        raise ThermalPrinterValueError("[BarCode.ITF] len(data) must be even.")
...
def validate_barcode_position(position: BarCodePosition) -> None:
    ...
    if not isinstance(position, BarCodePosition):
        ...
        raise ThermalPrinterConstantError(f"Valid positions are: {err}.")
...
def validate_chinese_format(fmt: Chinese) -> None:
    ...
    if not isinstance(fmt, Chinese):
        ...
        raise ThermalPrinterConstantError(f"Valid Chinese formats are: {err}.")
...
def validate_codepage(codepage: CodePage) -> None:
    ...
    raise ThermalPrinterConstantError(f"Valid codepages are: {codes}")
